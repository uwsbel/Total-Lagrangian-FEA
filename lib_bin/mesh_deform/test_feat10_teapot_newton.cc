/**
 * FEAT10 Teapot Newton Mesh Deformation Demo
 *
 * Mirrors the structure of the FEAT10 bunny Newton demo, but loads the FEAT10
...
const SolidMaterialProperties mat_teapot =
    SolidMaterialProperties::SVK(2.0e6,   // E: Young's modulus (Pa)
...
int main(int argc, char** argv) {
  ...
  cli.SetDescription(
      "FEAT10 teapot Newton mesh deformation demo with selectable mesh "
      "resolution.");
  cli.AddInt("res", 0,
             "teapot mesh resolution in data/meshes/T10/teapot_scaling "
             "(allowed: 0, 2, 4, 8, 16)");
  ...
  const std::string mesh_prefix =
      "data/meshes/T10/teapot_scaling/teapot_res" + std::to_string(res) + ".1";
  ...
  if (n_nodes <= 0 || n_elems <= 0) {
    std::cerr << "Failed to read teapot mesh for --res=" << res << "."
              << std::endl;
    return 1;
  }
  ...
  const double z_top = z_min + 0.8 * z_rng;
  ...
  constexpr double total_force_z = 1000.0;  // N
  ...
  for (int step = 0; step < 1000; ++step) {
    ...
    if (step % output_interval == 0) {
      ...
      gpu_t10_data.WriteOutputVTU("output/teapot_newton_res" +
