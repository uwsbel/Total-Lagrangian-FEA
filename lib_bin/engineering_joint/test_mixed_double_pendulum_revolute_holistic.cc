/**
 * Mixed ANCF3243 + FEAT10 Double Pendulum — Revolute Both Ends (Holistic)
 *
 * Upper link:  ANCF3243 beam.
 * Lower link:  FEAT10 T10 block.
 *
 * World anchor: revolute joint (3 CD + 2 WorldDP1 rows) with hinge axis
 *               aligned with the beam's cross-section slope r_v.
 * Elbow:        revolute joint (3 CD + 2 DP1 rows) between ANCF tip and
 *               T10 hinge with hinge axis along the shared body-Y direction.
 *
 * ANCF-side bindings follow Sugiyama, Escalona & Shabana (2003)
 * "Formulation of Three-Dimensional Joint Constraints Using the Absolute
 * Nodal Coordinates":  the secondary point q is built from the nodal r_v
 * slope DOF via MakeANCF3243SlopeBinding so that (q - p) = eps * r_v.
 * Using r_v as the hinge-axis vector makes the DP1 row
 * (q - p) · direction = 0 reduce to r_v · direction = 0.
 *
 * T10-side bindings use LocateReferencePoint — the T10 locator is reliable.
 * Offset points on the T10 block (perpendicular to the hinge axis) are
 * located at hinge_point + eps * perp_direction.
 *
 * Output:
 *   output/engineering_joint/mixed_double_pendulum_revolute_holistic_upper_XXXXXX.vtu
 *   output/engineering_joint/mixed_double_pendulum_revolute_holistic_lower_XXXXXX.vtu
 */
...
// Pose — matches test_feat10_double_pendulum_revolute_holistic (2D).
// No out-of-plane tilt so the pendulum swings in the XZ plane only.  Hinge
// axis is world UnitY at both joints.
...
constexpr double kUpperOutOfPlaneDeg   = 0.0;
constexpr double kLowerOutOfPlaneDeg   = 0.0;
...
std::string MakeOutputPath(const std::string& body_name, int frame) {
  ...
  oss << "output/engineering_joint/mixed_double_pendulum_revolute_holistic_"
...
int main(int argc, char** argv) {
  ...
  std::cout << "========================================\n";
  std::cout << "Mixed ANCF+T10 — Revolute Both Ends (Holistic)\n";
  std::cout << "========================================\n";
  ...
  const MixedConstraintPointBinding q_world =
      constraints.MakeANCF3243SlopeBinding(upper_block, 0,
                                           /*slope_slot=*/2, kSecondaryEps);
  const MixedConstraintPointBinding q_elbow_upper =
      constraints.MakeANCF3243SlopeBinding(upper_block, beam_n_nodes - 1,
                                           /*slope_slot=*/2, kSecondaryEps);

  // Hinge axis is R_upper·UnitY (image of beam +Y in world); perpendiculars
  // are R_upper·UnitX and R_upper·UnitZ.
  const Eigen::Vector3d perp1_world = R_upper * Eigen::Vector3d::UnitX();
  const Eigen::Vector3d perp2_world = R_upper * Eigen::Vector3d::UnitZ();

  const MixedConstraintPointBinding p_elbow_lower =
      constraints.LocateReferencePoint(lower_block, lower_hinge);
  const MixedConstraintPointBinding s_elbow_lower =
      constraints.LocateReferencePoint(lower_block,
                                       lower_hinge + kSecondaryEps * perp1_world);
  const MixedConstraintPointBinding t_elbow_lower =
      constraints.LocateReferencePoint(lower_block,
                                       lower_hinge + kSecondaryEps * perp2_world);

  // World anchor: 3 CD + 2 WorldDP1 (hinge axis ∥ R_upper·UnitY).
  ...
  constraints.AddWorldDP1Constraint(p_world, q_world, perp1_world, 0.0);
  constraints.AddWorldDP1Constraint(p_world, q_world, perp2_world, 0.0);

  // Elbow: 3 CD + 2 DP1 (ANCF r_v ⊥ two T10 perpendiculars).
  constraints.AddPointToPointCDAxis(p_elbow_upper, p_elbow_lower, 0);
  constraints.AddPointToPointCDAxis(p_elbow_upper, p_elbow_lower, 1);
  constraints.AddPointToPointCDAxis(p_elbow_upper, p_elbow_lower, 2);
  constraints.AddDP1Constraint(p_elbow_upper, q_elbow_upper,
                               p_elbow_lower, s_elbow_lower, 0.0);
  constraints.AddDP1Constraint(p_elbow_upper, q_elbow_upper,
                               p_elbow_lower, t_elbow_lower, 0.0);

  constraints.Finalize();

  std::cout << "bindings:\n";
  PrintBinding("p_world       (ANCF root)", p_world);
  PrintBinding("q_world       (pos + eps*r_v)", q_world);
  PrintBinding("p_elbow_upper (ANCF tip) ", p_elbow_upper);
  PrintBinding("q_elbow_upper (tip + eps*r_v)", q_elbow_upper);
  PrintBinding("p_elbow_lower (T10)      ", p_elbow_lower);
  PrintBinding("s_elbow_lower (T10+eps·perp1)", s_elbow_lower);
  PrintBinding("t_elbow_lower (T10+eps·perp2)", t_elbow_lower);
