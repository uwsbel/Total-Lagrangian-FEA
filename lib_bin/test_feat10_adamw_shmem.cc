...
int main() {
  ...
  // Use the shared memory optimized solver
  ...
  // Run 5 batches of 10 steps each (total 50 steps)
  for (int batch = 0; batch < 5; batch++) {
    std::cout << "\n----- Batch " << batch << " (10 steps) -----" << std::endl;
    solver.Solve(10);  // 10 steps per kernel launch
  }
