...
# tire.ele uses standard ordering (confirmed by midside-node distance heuristic)
...
if rank == 0:
    print(f"Loaded mesh: tire (standard ordering)")
...
vtk_output_dir = os.path.join(root_output_dir, "tire_fenics_vtk")
if rank == 0:
...
for n in range(n_steps):
    ...
    if WRITE_VTK and n % vtk_interval == 0:
        vtk_path = os.path.join(vtk_output_dir, f"tire_fenics_frame_{output_frame:04d}.vtk")
