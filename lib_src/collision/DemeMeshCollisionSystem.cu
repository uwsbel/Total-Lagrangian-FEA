...
static void ConfigureDemeRuntimePathsFromBazelBin() {
  ...
  //
  // The `external/` dir lives under the `bin/` root, not next to the executable,
  // so search upward from the executable directory for a matching install tree.
  ...
  for (fs::path p = exe.parent_path(); !p.empty(); p = p.parent_path()) {
    if (try_set_from_install(p / install_rel)) return;
  }

  // Fallback: common symlink location in the workspace when launched as
  // `./bazel-bin/...`.
  if (try_set_from_install(fs::path("bazel-bin") / install_rel)) return;
}
...
static float EnvFloatOrFallback(const char* primary,
                                const char* fallback,
                                float default_value) {
  const float nan = std::numeric_limits<float>::quiet_NaN();
  const float v_primary = EnvFloatOr(primary, nan);
  if (std::isfinite(v_primary)) return v_primary;
  const float v_fallback = EnvFloatOr(fallback, nan);
  if (std::isfinite(v_fallback)) return v_fallback;
...
static float3 ToFloat3(const Eigen::Vector3d& v) {
  return make_float3(static_cast<float>(v.x()),
...
static float3 MeanOf(const std::vector<float3>& nodes) {
  if (nodes.empty()) return make_float3(0.f, 0.f, 0.f);
  ...
  return make_float3(static_cast<float>(sx * inv),
...
static void AccumulatePointForcesToKNearestNodes(
  ...
  for (size_t i = 0; i < points.size() && i < forces.size(); ++i) {
    ...
    for (int vi = 0; vi < static_cast<int>(mesh_vertices.size()); ++vi) {
      const double dx = static_cast<double>(mesh_vertices[vi].x) - static_cast<double>(points[i].x);
      const double dy = static_cast<double>(mesh_vertices[vi].y) - static_cast<double>(points[i].y);
      const double dz = static_cast<double>(mesh_vertices[vi].z) - static_cast<double>(points[i].z);
...
static void ExtendBBox(Eigen::Vector3d& mn,
                       Eigen::Vector3d& mx,
...
DemeMeshCollisionSystem::DemeMeshCollisionSystem(
  ...
  for (auto& body : bodies) {
    ...
    if (surf.vertices.size() != surf.global_node_ids.size()) {
      throw std::invalid_argument(
          "DemeMeshCollisionSystem: SurfaceTriMesh vertices size must match global_node_ids size");
    }
    if (surf.vertices.empty() || surf.triangles.empty()) {
      throw std::invalid_argument(
          "DemeMeshCollisionSystem: each body must have non-empty surface vertices and triangles");
    }
    const int n_verts = static_cast<int>(surf.vertices.size());
    for (const auto& tri : surf.triangles) {
      if (tri.x() < 0 || tri.y() < 0 || tri.z() < 0 ||
          tri.x() >= n_verts || tri.y() >= n_verts || tri.z() >= n_verts) {
        throw std::invalid_argument(
            "DemeMeshCollisionSystem: SurfaceTriMesh triangle indices out of range");
      }
    }
...
void DemeMeshCollisionSystem::BuildSolver(double friction) {
  ...
  for (const auto& rb : bodies_) {
    for (const auto& v : rb.body.surface.vertices) ExtendBBox(mn, mx, v);
  }

  const double pad = 1.0;
  solver_->InstructBoxDomainDimension({static_cast<float>(mn.x() - pad), static_cast<float>(mx.x() + pad)},
  ...
  auto make_mesh = [&](const ANCFCPUUtils::SurfaceTriMesh& surf,
                       unsigned int family,
                       bool split_into_patches,
                       float patch_angle_deg,
                       const char* label) {
    deme::DEMMesh mesh;
    mesh.SetFamily(family);

    // DEME expects mesh vertices in the owner's local frame. Keep the owner init
    // position meaningful by centering vertices at the initial centroid.
    ...
    for (const auto& tri : surf.triangles) {
      mesh.m_face_v_indices.push_back(
          make_int3(tri.x(), tri.y(), tri.z()));
    }
    ...
    if (split_into_patches) {
      // Smaller angle => more patches => more mesh-mesh contact points (patch pairs).
      // For highly concave meshes, this is important: DEME's patch-level contact
      // can otherwise collapse to ~one contact point.
      ...
      if (angle > 0.0f && angle < 360.0f) {
        ...
        constexpr unsigned int kMaxPatches =
            static_cast<unsigned int>(std::numeric_limits<deme::patchID_t>::max()) + 1u;

        float try_angle = angle;
        unsigned int n_patches = 0;
        for (int attempt = 0; attempt < 6; ++attempt) {
          n_patches = mesh.SplitIntoConvexPatches(try_angle);
          if (n_patches > 0 && n_patches <= kMaxPatches) break;
          if (try_angle >= 180.0f) break;
          try_angle = std::min(180.0f, try_angle * 1.5f);
        }

        if (n_patches == 0) {
          std::cout << "[DEME] Patch splitting produced 0 patches for " << label
                    << " (angle_threshold_deg=" << try_angle << "); keeping single patch\n";
          mesh.SetPatchIDs(std::vector<deme::patchID_t>(mesh.nTri, 0));
        } else if (n_patches > kMaxPatches) {
          std::cout << "[DEME] Patch splitting produced too many patches for " << label
                    << " (" << n_patches << " > " << kMaxPatches
  ...
  const float default_patch_angle_deg =
      EnvFloatOrFallback("DEME_PATCH_ANGLE_DEG", "DEME_ITEM_PATCH_ANGLE_DEG", 20.0f);

  // Family usage is global in DEME; self-collision can only be toggled per-family.
  std::unordered_map<unsigned int, int> family_counts;
  for (const auto& rb : bodies_) family_counts[rb.body.family] += 1;

  std::vector<unsigned int> unique_families;
  unique_families.reserve(family_counts.size());
  for (const auto& it : family_counts) unique_families.push_back(it.first);
  ...
  if (!enable_self_collision_) {
    for (const auto& it : family_counts) {
      ...
      if (count == 1) {
        ...
        std::cout << "[DEME] Family " << fam << " is shared by " << count
                  << " bodies; cannot disable self-collision without also disabling "
                     "cross-body contacts in that family. Leaving self-contact enabled.\n";
      }
    }
  }
  ...
  for (size_t bi = 0; bi < bodies_.size(); ++bi) {
    ...
    const float angle = (body.patch_angle_deg > 0.0f) ? body.patch_angle_deg : default_patch_angle_deg;
  ...
  // Keep all mesh families fully prescribed (we will update their nodes directly).
  ...
  // don't accidentally use `NULL_BODYID` (which would segfault in DEME setters).
  for (auto& rb : bodies_) {
    if (!rb.mesh_handle) {
      throw std::runtime_error("DemeMeshCollisionSystem: mesh handle is null after AddMesh");
    }
    if (rb.mesh_handle->owner == deme::NULL_BODYID) {
      throw std::runtime_error(
          "DemeMeshCollisionSystem: DEME mesh owner ID was not assigned after Initialize()");
    }
...
void DemeMeshCollisionSystem::BindNodesDevicePtr(double* d_nodes_xyz,
  ...
  if (d_f_contact_ == nullptr) {
    HANDLE_ERROR(cudaMalloc(&d_f_contact_, static_cast<size_t>(3 * n_nodes_) * sizeof(double)));
  }
}
...
void DemeMeshCollisionSystem::Step(const CollisionSystemInput& in,
  ...
  if (d_nodes_xyz_ == nullptr || n_nodes_ <= 0) {
    throw std::runtime_error("DemeMeshCollisionSystem::Step called before BindNodesDevicePtr");
  }
  if (in.dt <= 0.0) {
    throw std::invalid_argument("DemeMeshCollisionSystem::Step: dt must be > 0");
  }
  ...
  for (size_t bi = 0; bi < bodies_.size(); ++bi) {
    ...
    for (size_t i = 0; i < surf.global_node_ids.size(); ++i) {
      nodes[i] = Vec3FromNodesXYZ(h_nodes_xyz_, n_nodes_, surf.global_node_ids[i]);
    }

    const float3 com = MeanOf(nodes);
    if (rb.owner == static_cast<unsigned int>(deme::NULL_BODYID)) {
      throw std::runtime_error("DemeMeshCollisionSystem::Step: DEME mesh owner ID is NULL_BODYID");
    }
  ...
  // - `DEME_FORCE_CLAMP`: clamps each contact force vector's norm (N); 0 disables.
  ...
  for (size_t bi = 0; bi < bodies_.size(); ++bi) {
    ...
    AccumulatePointForcesToKNearestNodes(nodes, surf.global_node_ids, points, forces,
                                         k, force_scale, clamp_norm, h_f_contact_);
  }
