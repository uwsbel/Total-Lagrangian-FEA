...
__device__ double solver_grad_L(int tid, ElementBase *d_data, SyncedAdamWSolver *d_solver)
{
    double res = 0.0;
...
__global__ void
one_step_adamw_kernel(ElementBase *d_data, SyncedAdamWSolver *d_adamw_solver)
{
    ...
    for (int outer_iter = 0; outer_iter < d_adamw_solver->solver_max_outer(); outer_iter++)
    {
        if (*d_adamw_solver->outer_flag() == 0)
        {
            ...
            for (int inner_iter = 0; inner_iter < d_adamw_solver->solver_max_inner(); inner_iter++)
            {
                ...
                if (*d_adamw_solver->inner_flag() == 0)
                {
                    ...
                    if (tid < d_adamw_solver->get_n_coef() * 3)
                    {
                        ...
                        v_t = beta2 * v_t + (1 - beta2) * d_adamw_solver->g()(tid) * d_adamw_solver->g()(tid);
                        double m_hat = m_t / (1 - pow(beta1, t));
                        double v_hat = v_t / (1 - pow(beta2, t));
                        y = d_adamw_solver->v_guess()(tid) - lr * (m_hat / (sqrt(v_hat) + eps) + weight_decay * d_adamw_solver->v_guess()(tid));

                        // Store look-ahead velocity temporarily
                        d_adamw_solver->v_guess()(tid) = y; // Use v_guess as temp storage for y
                    }
                    ...
                    if (tid < d_adamw_solver->get_n_coef())
                    {
                        ...
                        {
                            auto *data = static_cast<GPU_ANCF3243_Data *>(d_data);
                            data->x12()(tid) = d_adamw_solver->x12_prev()(tid) + d_adamw_solver->solver_time_step() * d_adamw_solver->v_guess()(tid * 3 + 0);
                            data->y12()(tid) = d_adamw_solver->y12_prev()(tid) + d_adamw_solver->solver_time_step() * d_adamw_solver->v_guess()(tid * 3 + 1);
                            data->z12()(tid) = d_adamw_solver->z12_prev()(tid) + d_adamw_solver->solver_time_step() * d_adamw_solver->v_guess()(tid * 3 + 2);
                        }
                        else if (d_data->type == TYPE_3443)
                        {
                            auto *data = static_cast<GPU_ANCF3443_Data *>(d_data);
                    ...
                    if (tid == 0)
                    {
                        ...
                        *d_adamw_solver->norm_g() = sqrt(norm_g);
                        ...
                        if (*d_adamw_solver->norm_g() <= d_adamw_solver->solver_inner_tol() * (1.0 + norm_v_curr))
                        {
            ...
            if (tid < d_adamw_solver->get_n_coef())
            {
                ...
                {
                    auto *data = static_cast<GPU_ANCF3243_Data *>(d_data);
                    data->x12()(tid) = d_adamw_solver->x12_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 0) * d_adamw_solver->solver_time_step();
                    data->y12()(tid) = d_adamw_solver->y12_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 1) * d_adamw_solver->solver_time_step();
                    data->z12()(tid) = d_adamw_solver->z12_prev()(tid) + d_adamw_solver->v_guess()(tid * 3 + 2) * d_adamw_solver->solver_time_step();
                }
                else if (d_data->type == TYPE_3443)
                {
                    auto *data = static_cast<GPU_ANCF3443_Data *>(d_data);
            ...
            if (tid == 0)
            {
                ...
                for (int i = 0; i < d_adamw_solver->gpu_n_constraints(); i++)
                {
                    ...
                    d_adamw_solver->lambda_guess()[i] += *d_adamw_solver->solver_rho() * d_adamw_solver->solver_time_step() * constraint_val;
                }
...
void SyncedAdamWSolver::OneStepAdamW()
{
    ...
    HANDLE_ERROR(cudaLaunchCooperativeKernel((void *)one_step_adamw_kernel, blocks, threads, args));
    ...
    std::cout << "OneStepAdamW kernel time: " << milliseconds << " ms" << std::endl;
