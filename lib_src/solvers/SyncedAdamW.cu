...
// Templated kernel - uses global memory for grad_N/grad_s
template <typename ElementType>
    ...
    if (*d_adamw_solver->outer_flag() == 0) {
      ...
      for (int inner_iter = 0; inner_iter < d_adamw_solver->solver_max_inner();
        ...
        if (*d_adamw_solver->inner_flag() == 0) {
          ...
          // Compute P (stress) - using global memory for grad_N/grad_s
          // Use grid-stride loop since n_elem * n_qp may exceed 3 * n_coef
          {
            const int n_elem = d_adamw_solver->get_n_beam();
            const int n_qp = d_adamw_solver->gpu_n_total_qp();
            const int total_work = n_elem * n_qp;
            
            for (int idx = tid; idx < total_work; idx += grid.size()) {
              int elem_idx = idx / n_qp;
              int qp_idx   = idx % n_qp;
          ...
          // Compute internal force - using global memory for grad_N/grad_s
          // Use grid-stride loop since n_elem * n_qp may exceed 3 * n_coef
          {
            const int n_elem = d_adamw_solver->get_n_beam();
            const int n_qp = d_adamw_solver->gpu_n_total_qp();
...
void SyncedAdamWSolver::OneStepAdamW() {
  ...
  // No shared memory needed - using global memory for grad_N/grad_s
  if (type_ == TYPE_3243) {
    int threads = 256;
    int N            = 3 * n_coef_;
