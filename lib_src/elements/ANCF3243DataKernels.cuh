...
__device__ __forceinline__ double det3x3(const double *J)
{
    return J[0] * (J[4] * J[8] - J[5] * J[7]) - J[1] * (J[3] * J[8] - J[5] * J[6]) + J[2] * (J[3] * J[7] - J[4] * J[6]);
}

__device__ __forceinline__ void b_vec(double u, double v, double w, double *out)
{
    out[0] = 1.0;
    out[1] = u;
    out[2] = v;
...
__device__ __forceinline__ void b_vec_xi(double xi, double eta, double zeta, double L, double W, double H, double *out)
{
    double u = L * xi / 2.0;
    double v = W * eta / 2.0;
    double w = H * zeta / 2.0;
...
__device__ __forceinline__ void calc_det_J_xi(double xi,
                                              double eta,
                                              double zeta,
