  ...
  for (int e = 0; e < n_beam; ++e) {
    for (int qp = 0; qp < n_qp; ++qp) {
      std::cout << "\n=== Elem " << e << " Quadrature Point " << qp
                << " detJ_ref=" << h_detJ[e * n_qp + qp] << " ===" << std::endl;
...
void GPU_ANCF3443_Data::BuildMassCSRPattern() {
...

  {
    ...
    decode_mass_keys_3443_kernel<<<blocks, threads>>>(
        d_keys, nnz, d_csr_columns, d_row_counts);
    HANDLE_ERROR(cudaDeviceSynchronize());
  }
...
__global__ void build_constraint_jt_row_counts_3443_kernel(
    int n_constraint, const int *fixed_nodes, int *row_counts) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid >= n_constraint) {
    return;
  }
...
__global__ void build_constraint_jt_fill_3443_kernel(
    int n_constraint, const int *fixed_nodes, const int *offsets,
    int *row_positions, int *columns, double *values) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid >= n_constraint) {
    return;
  }
...
void GPU_ANCF3443_Data::RetrieveConstraintJacobianCSRToCPU(
